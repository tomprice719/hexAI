...
while len(games) < games_required:
    ...
    game_makers += [GameMaker() for i in range(batch_size - len(game_makers))]
    ...
    win_prob = model.predict(positions)

    position_counter = 0
    for g in game_makers:
        g.update(win_prob[position_counter: position_counter + g.num_positions_required()])
